...
def conversation_reply(system_prompt: str, input_structure: dict, runnable_config: dict, memory: ConversationBufferMemory) -> tuple[str, list[str]]:
    """
    Conducts a conversation using a language model chain, generates a response, and retrieves references.

    Parameters:
        system_prompt (str): The system prompt to start the conversation.
        input_structure (dict): Structure for conversation input.
        runnable_config (dict): Runnable configuration.
        memory (ConversationBufferMemory): Memory for conversation history.

    Returns:
        Tuple[str, List[str]]: A tuple containing the full response text and a list of references.
    """
    ...
    # Invoke conversation chain and retrieve response along with references
    refs = ['book 1.pdf', 'book 2.pdf', 'book 3.pdf']
    full_response = chain.invoke(input_structure, config=runnable_config)["text"]

    return full_response, refs
...
if prompt := st.chat_input(placeholder="Ask anything you want!"):
    if len(prompt) > MAX_CHAR_LIMIT:
        ...
        with st.chat_message("assistant", avatar="🤖"): #
            ...
            full_response, ref = conversation_reply(system_prompt, input_structure, runnable_config, memory)
...
if st.session_state.get("run_id"):
    ...
    if feedback:
        ...
        if score is not None:
            ...
            # TODO: add to S3 bucket code here
